...
def generate_random_63_orders(version, seed):
    ...
    if version == 'trial':
        ...
        jobs = [1, 2, 3]
        resources = [63, red, green]
        quantity = []
        for i in range(len(b) + 1):
            if i == 0:
                quantity.append(b[i])
            elif i == 2:
                quantity.append(63-b[-1])
            else:
                quantity.append(b[i] - b[i-1])
        ...
        jobs = [i + 1 for i in range(15)]
        resources = [63, red, green, blue, violet]
        quantity = []
        for i in range(len(b)+1):
            if i == 0:
                quantity.append(b[i])
...
class ConveyorEnv_v4(gym.Env):
    ...
    def _RESET(self):
        ...
        self.jobs, self.res, self.quantity = generate_random_63_orders(self.version, self.seed)
        if self.version == 'trial':
